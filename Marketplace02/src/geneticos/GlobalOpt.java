...
public class GlobalOpt {
	...
	public GlobalOpt(double[] param, int[] tipoNodo, double[] restr, int[] iter, double[] prob, ArrayList ofertas, int[] bundling){
		this.setIter(iter);
		this.setParam(param);
		this.setProb(prob);
	...
	public FitnessFunction createFitnessFunction() { 
	    return new GlobalOptFitnessFunction(this.param, this.tipoNodo, this.restr, this.iter, this.prob, this.ofertas, this.bundling); 
	}
	...
	public IChromosome servOptimos() throws Exception {
		...
		while(cont<serv){
			...
			compGene.setApplicationData(ofertas[asigno[cont]][9]+"_"+ofertas[asigno[cont]][10]+"_"+ofertas[asigno[cont]][11]+"_"+ofertas[asigno[cont]][13]);
